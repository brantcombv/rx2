...
class Rx_Crypt_Adapter_Xor_Advanced extends Rx_Crypt_Adapter_Abstract
{
    /**
    ...
    {
        $key = $params['key'];
        $aXor = ord(substr($key, 0, 1));
        $aInc = max(ord(substr($key, 1, 1)), 1);
        $sz = strlen($key);
        $kPos = 0;
        $cPos = 0;
        ...
        $result = '';
        while ($count--) {
            $result .= chr(ord($content[$cPos++]) ^ ord($key[$kPos++]) ^ $aXor);
            $kPos = $kPos % $sz;
            $aXor = ($aXor + $aInc) % 0xFF;
        }
        return ($result);
